export const EXAMPLE_FORMAT = "hh:mm:ss.xxx (hours and minutes are optional)";
...
export class TrackmaniaTime {
	...
	public getTotalInMilliSeconds() {
		return this.milliseconds + 1000 * this.seconds + 1000 * 60 * this.minutes + 1000 * 60 * 60 * this.hours;
	}

	public toString() {
		const padZero = (num: number, size: number) => num.toString().padStart(size, "0");

		const milliseconds = padZero(this.milliseconds, 3);
		const seconds = padZero(this.seconds, 2);
		const minutes = padZero(this.minutes, 2);

		if (this.hours > 0) return `${this.hours}:${minutes}:${seconds}.${milliseconds}`;
